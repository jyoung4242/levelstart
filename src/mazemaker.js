/** Creates a w x h maze with corridors of value 0 and walls of 255,
    ...
    BSD License
*/
export function makeMaze(w, h, horizontal, vertical, straightness, imperfect, fill, deadEndArray, hallWidth, wallWidth, random) {
  ...
  if (horizontal.loop && horizontal.border && horizontal.symmetry) {
    ...
    for (let y = 1; y < maze[0].length; y += 2) {
      ...
      if (maze[1][y] === EMPTY && maze[0][y - 2] !== EMPTY && random() < prob) {
        maze[0][y] = EMPTY;
  ...
  if (vertical.loop && vertical.border && vertical.symmetry) {
    ...
    for (let x = 1; x < maze.length; x += 2) {
      ...
      if (maze[x][1] === EMPTY && (x < 2 || maze[x - 2][0] !== EMPTY) && random() < prob) {
        maze[x][0] = EMPTY;
...
/** 
    ...
    dead ends.
 */
export function addMapRooms(map, hallWidth, wallWidth, horizontal, vertical, mazeDeadEndArray, roomsFraction) {
  const EMPTY = 0;
  const w = map.length,
    h = map[0].length,
    floor = Math.floor,
